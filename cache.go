...
func NewCache[K Key, V any](config *Config[K, V]) (*Cache[K, V], error) {
	...
	if cache.keyToHash == nil {
		cache.keyToHash = helpers.KeyToHash[K]
	}
	...
	// NOTE: benchmarks show that performance degrades as
	// the number of cache.processItems() goroutines running increases,
	// so a value of 1 is usually sufficient.
